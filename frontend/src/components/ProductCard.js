...
const ProductCard = ({ product, onRefresh, onDelete }) => {
  ...
  const { _id, title, image, url, currentPrice, initialPrice, targetPrice, priceHistory, currency } = product;

  const sym = currency || '$';

  const priceDiff    = currentPrice - initialPrice;
  const priceDropPct = initialPrice > 0 ? Math.abs((priceDiff / initialPrice) * 100).toFixed(1) : 0;
  ...
  return (
    <div
      ...
      className={`card flex flex-col overflow-hidden transition-all duration-300
                  ...
                  ${isBelowTarget ? 'ring-1 ring-brand-500/50 shadow-orange-sm' : ''}
      ...
      <div className="relative h-44 bg-gray-800 overflow-hidden">
        ...
        {hasTarget && (
          <div className={`absolute top-2 right-2 px-2.5 py-1 rounded-full text-xs font-bold
                          ...
                          ${isBelowTarget ? 'bg-brand-500/90 text-white' : 'bg-rose-500/90 text-white'}`}>
            {isBelowTarget ? (
              <>
        ...
        {/* Currency badge for non-USD */}
        {sym !== '$' && (
          <div className="absolute top-2 left-2 px-2 py-0.5 rounded-full text-xs font-bold
                          bg-gray-900/80 text-brand-400 border border-brand-500/30 backdrop-blur-sm">
            {sym}
          </div>
        )}
      </div>
      ...
      <div className="flex flex-col flex-1 p-4 gap-3">
        ...
        <div className="flex items-end justify-between">
          <div>
            ...
            <p className="price-tag">{currentPrice > 0 ? `${sym}${currentPrice.toFixed(2)}` : 'N/A'}</p>
          </div>
          <div className="text-right">
            <p className="text-xs text-gray-500 mb-0.5">Initial</p>
            <p className="text-sm text-gray-400 line-through">
              {initialPrice > 0 ? `${sym}${initialPrice.toFixed(2)}` : '—'}
            </p>
          </div>
        </div>
